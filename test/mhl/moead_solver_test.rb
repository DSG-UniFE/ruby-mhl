...
describe MHL::MOEADSolver do
  ...
  context 'convergence' do
    it 'should produce solutions approaching the true Pareto front on ZDT1' do
      ...
      # on ZDT1, the true Pareto front satisfies f2 = 1 - sqrt(f1),
      # with f1 in [0,1]. we measure each solution's distance to the
      # nearest point on the true front using generational distance (GD).
      distances = result.map do |solution|
        ...
        # distance to the true Pareto front at this f1 value
        f2_true = 1.0 - Math.sqrt([f1, 0.0].max)
        (f2 - f2_true).abs
      end
